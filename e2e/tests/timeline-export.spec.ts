...
/**
 * Helper to start screen share
 */
async function startScreenShare(page: Page) {
...
/**
 * Helper to record for a given duration
 */
...
/**
 * Helper to wait for export to complete and verify success
 * Returns 'success' or throws error with details
 */
async function waitForExportComplete(page: Page, timeout: number = 180000): Promise<'success'> {
  await page.waitForSelector(selectors.nle.exportingHeader, { timeout: 10000 });

  const result = await Promise.race([
    page.waitForSelector(selectors.nle.exportCompleteTitle, { timeout }).then(() => 'success' as const),
    page.waitForSelector(selectors.nle.exportFailedTitle, { timeout }).then(() => 'failed' as const),
  ]);
 ...
 */
async function getTransferState(page: Page) {
  return await page.evaluate(() => {
    const store = (
      window as unknown as Record<
        string,
 ...
 */
async function getRecordingState(page: Page) {
  return await page.evaluate(() => {
    const store = (
      window as unknown as Record<
        string,
...
test.describe('Timeline Export', () => {
  ...
  test('exports single-user recording with camera only', async () => {
    ...
    // Record for 3 seconds (camera only)
    ...
    // Go to NLE Editor and export
    await goToNLEEditor(page);
    ...
    await waitForExportComplete(page, 120000);
  ...
  test('exports single-user recording with camera and screen share', async () => {
    ...
    await waitForExportComplete(page, 180000);
    console.log('[Timeline Export] Export completed!');
  });
  ...
  test('export can be cancelled', async () => {
    // This test verifies the export can be cancelled mid-process
    app = await launchApp('timeline-export-cancel-' + Date.now());
    const { page } = app;

    await setupSessionAsHost(page, 'Cancel Test User');

    // Record briefly (2 seconds)
    console.log('[Export Cancel Test] Starting brief recording...');
    await recordForDuration(page, 2000);

    // Go to NLE Editor
    await goToNLEEditor(page);

    // Start export
    console.log('[Export Cancel Test] Starting export...');
    const exportButton = page.locator(selectors.nle.exportButton);
    await expect(exportButton).toBeEnabled({ timeout: 5000 });
    await exportButton.click();
